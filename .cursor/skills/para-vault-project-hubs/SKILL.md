...
1. **Folder project (features, clones, docs trees)** — `Projects/<slug>/` with **`MOC.md`** at that root as the **only** Obsidian index for that slug. Link **`context/`**, **`docs/`** (technical repo mount when present), `decisions/`, etc. See **`docs/` vs `context/`** in `.cursor/rules/moc-project-structure.mdc`. Never add `Projects/<slug>.md` alongside the same-named folder.
