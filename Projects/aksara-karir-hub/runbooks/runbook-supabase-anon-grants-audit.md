...
Verify whether the prod Supabase Postgres has over-broad grants on `public` tables for `anon` / `authenticated` (a side-effect of the local-dev SQL snippet in `AGENTS.md`). Required pre-step for [[Projects/aksara-karir-hub/decisions/adr-2026-04-26-rls-vs-typeorm-only]].
...
3. Check RLS status:
   ...
   ORDER BY tablename;
   ```
4. Save output to `Projects/aksara-karir-hub/runbooks/audit-results/<date>.md` (create folder if needed).
...
- [[Projects/aksara-karir-hub/decisions/adr-2026-04-26-rls-vs-typeorm-only]]
- [[Projects/aksara-karir-hub/backlog/p0-rls-or-revoke-anon]]
