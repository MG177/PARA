...
- **Obsidian project hubs:** feature or app work under `Projects/<name>/` should index the vault via **`MOC.md`** in that folder—not a duplicate `Projects/<name>.md`. Link areas with `area:` / `area/…` tags. Dashboard Dataview must use **`regexmatch(pattern, field)`** (pattern first). Folder layout: **`context/`** = vault-side client/onboarding/commercial notes (hub `context/index.md`); **`docs/`** = technical/product markdown with the code (submodule or copy). Full rules: `.cursor/rules/moc-project-structure.mdc`.
